...
export default function ProfileClient() {
  ...
  // discover signed-in username (client-side only)
  useEffect(() => {
    const u =
  ...
  // load current desc + avatar when username known
  useEffect(() => {
    if (!username) return;
  ...
  async function onPickAvatar(file: File) {
    if (!file) return;
    // cap file size we hold in localStorage (~ 500KB–ish via compression/resize)
